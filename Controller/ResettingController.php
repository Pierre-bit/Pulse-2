...
class ResettingController extends AbstractController
{
    ...
    public function request(Request $request, Mailer $mailer, TokenGeneratorInterface $tokenGenerator)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $request->getSession()->getFlashBag()->add('success', "Un mail va vous être envoyé afin que vous puissiez renouveller votre mot de passe. Le lien que vous recevrez sera valide 20 min.");
    ...
    // si supérieur à 20min, retourne false
    ...
    public function resetting(User $user, $token, Request $request, UserPasswordEncoderInterface $passwordEncoder)
    {
        ...
        // le token date de plus de 20 minutes
        if ($user->getToken() === null || $token !== $user->getToken() || !$this->isRequestInTime($user->getPasswordRequestedAt())) {
            // throw new AccessDeniedHttpException();
            $request->getSession()->getFlashBag()->add('Attention', "Le lien à expiré.");
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $em->persist($user);
            $em->flush();
            $request->getSession()->getFlashBag()->add('Succes', "Votre mot de passe a été renouvelé.");
